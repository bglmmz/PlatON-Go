...
func saveTransData(interpreter *EVMInterpreter, inputData, from, to []byte, code string) {
	...
	transKey := stats.TransHashName + numStr + txHash
	data, err := stats.Instance().Get([]byte(blockKey))
	...
	if flag {
		...
		stats.Instance().Put([]byte(blockKey), transBlockByte)
	}

	var transInput staking.TransInput
	transData, err := stats.Instance().Get([]byte(transKey))
	if nil != err {
	...
	stats.Instance().Put([]byte(transKey), transHashByte)
